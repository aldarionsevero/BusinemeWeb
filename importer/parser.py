...
from models.models import Busline, Terminal, Company
...
class Parser(object):
    ...
    def create_busline_terminal_relation(self):
        ...
        buslines = Busline.objects.all()
        terminals = Terminal.objects.all()
        for row in csv_file:
            bus_line = Busline()
            try:
                bus_line = buslines.get(line_number=row[0])
            except ObjectDoesNotExist:
                print 'Busline', row[0], 'does not exist.'

            try:
                terminal1 = terminals.get(description=row[5])
            ...
            try:
                terminal2 = terminals.get(description=row[6])
